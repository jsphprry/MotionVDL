...
public class Video extends Encoding {
	...
	public Video getCropped(int x, int y, int w, int h) {
		...
		if (0 > x || 0 > y || x >= width || y >= height)   throw new IllegalArgumentException(String.format("Video error: invalid crop coordinate '(%d,%d)'",x,y));
		if (0 > w || 0 > h || w > width-x || h > height-y) throw new IllegalArgumentException(String.format("Video error: invalid crop resolution '%dx%d'",w,h));
